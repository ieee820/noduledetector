...
def compute_mean_ci(interp_tps, confidence = 0.95):
    sens_mean = np.zeros((interp_tps.shape[1]),dtype = 'float32')
    sens_lb   = np.zeros((interp_tps.shape[1]),dtype = 'float32')
    sens_up   = np.zeros((interp_tps.shape[1]),dtype = 'float32')
    
    Pz = (1.0-confidence)/2.0
        
    for i in range(interp_tps.shape[1]):
        # get sorted vector
        vec = interp_tps[:,i]
        vec.sort()
...
def computeFROC_bootstrap(FROCGTList,FROCProbList,FPDivisorList,FROCImList,numberOfBootstrapSamples=1000, confidence = 0.95):

    set1 = np.concatenate(([FROCGTList], [FROCProbList]), axis=0)
    ...
    for i in range(numberOfBootstrapSamples):
        ...
        fps, tps, thresholds = computeFROC(btpsamp[0,:],btpsamp[1,:],FROCImList)
        
        fps_lists.append(fps)
        tps_lists.append(tps)
    ...
    interp_tps = np.zeros((numberOfBootstrapSamples,len(all_fps)), dtype = 'float32')
    for i in range(numberOfBootstrapSamples):
        interp_tps[i,:] = np.interp(all_fps, fps_lists[i], tps_lists[i])
    
    # compute mean and CI
    sens_mean,sens_lb,sens_up = compute_mean_ci(interp_tps, confidence = confidence)

    return all_fps, sens_mean, sens_lb, sens_up

def computeFROC(FROCGTList, FROCProbList, FROCImList):
    spec, sens, thresholds = skl_metrics.roc_curve(FROCGTList, FROCProbList)
    fps = spec * len(FROCProbList) / len(FROCImList)
...
def evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules, CADSystemName, maxNumberOfCADMarks=-1,
        ...
        for noduleAnnot in noduleAnnots:
            ...
            if noduleAnnot.state == "Included":
                ...
                if found == True:
                    ...
                    bootstrapFactorList.append(seriesuid)
                    ...
                    FROCGTList.append(1.0)
                    FROCProbList.append(sys.float_info.min)
                    FPDivisorList.append(seriesuid)
                    bootstrapFactorList.append(seriesuid)
    ...
    if not (len(FROCGTList) == len(FROCProbList) and len(FROCGTList) == len(FPDivisorList) and len(FROCGTList) == len(FROCtoNoduleMap)):
    ...
    # compute FROC
    fps, tps, thresholds = computeFROC(FROCGTList,FROCProbList,seriesUIDs)
    
    if performBootstrapping:
        fps_itp,sens_mean,sens_lb,sens_up = computeFROC_bootstrap(FROCGTList,FROCProbList,FPDivisorList,seriesUIDs,
    ...
    # cut off the last point because the FROC class adds 1.0 sensitivity at the end (which makes no sense)
    fps2 = fps[:-1]
    sens2 = tps[:-1]
    thresholds2 = thresholds[:-1]
    ...
    if performBootstrapping:
        ...
        with open(os.path.join(outputDir, "froc_%s_bootstrapping.csv" % CADSystemName), 'w') as f:
            ...
            for i in range(len(fps_itp)):
                f.write("%.9f,%.9f,%.9f,%.9f\n" % (fps_itp[i], sens_mean[i], sens_lb[i], sens_up[i]))
    else:
        fps_itp = None
        sens_mean = None
        sens_lb = None
    ...
    if int(totalNumberOfNodules) > 0:
        ...
        ax = plt.gca()
        clr = 'b'
        plt.plot(fps2, sens2, color=clr, label="%s" % CADSystemName, lw=2)
        if performBootstrapping:
            plt.plot(fps_itp, sens_mean, label="mean")
            plt.plot(fps_itp, sens_lb, color=clr, ls=':') # , label = "lb")
            plt.plot(fps_itp, sens_up, color=clr, ls=':') # , label = "ub")
            ax.fill_between(fps_itp, sens_lb, sens_up, facecolor=clr, alpha=0.05)
        xmin = FROC_minX
        xmax = FROC_maxX
        plt.xlim(xmin, xmax)
        plt.ylim(0, 1)
        ...
        plt.title('FROC Analysis %s %s' % (CADSystemName, graphTitle))
    ...
    return (fps2, sens2, thresholds2, fps_itp, sens_mean, sens_lb, sens_up)
